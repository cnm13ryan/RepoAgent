...
class GitignoreChecker:
    ...
    def _parse_gitignore(gitignore_content: str) -> list:
        """
        Parse the .gitignore content and return patterns as a list, ignoring blank
    ...
    def _split_gitignore_patterns(gitignore_patterns: list) -> tuple:
        ...
        for p in gitignore_patterns:
            if p.endswith("/"):
                folder_patterns.append(p.rstrip("/"))
                continue
    ...
    def check_files_and_folders(self) -> list:
        ...
        for root, dirs, files in os.walk(self.directory):
            ...
            for filename in files:
                ...
                # Skip ignored files
                if self._is_ignored(filename, self.file_patterns):
                    continue
                # Only keep .py files
